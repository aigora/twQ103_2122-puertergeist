...
int Banner(void)
{
 ...
 printf("           ||      ||   || ||      ||  ||    ||   ||      ||  ||  ||  ||  ||        ||        ||   ||   \n");
 printf("           ||       |||||  ||||||| ||   ||   ||   ||||||| ||   || ||||||  ||||||| |||||| |||||||   ||   \n");
 printf("\n \n \n           La sabiduria no se halla en entender el misterio, sino en aceptar que no se puede enteder.\n \n");
 printf("                                             ''Gregory Maguire''");
 printf("\n \n \n \n \n \n \n \n \n Pulse cualquier tecla para continuar");
getch();
system("cls");
 printf("Bienvenido a Puertergeist, en este juego pondras a prueba tu conocimiento sobre el mundo del terror.\n"); 
 //delay(2000)) */
 printf("Podras jugar en modo individual o multijugador ademas de tener acceso a la tabla de mejores puntuaciones");
 getch();
system("cls");
}

/*int Banner_Salir(void) no es necesario porque el banner lleva directamente al menu de juego
{
